...
class Notepad:
    ...
    def set_file_menu_event_bindings(self):
        ...
        self.root.bind("<Control-d>", lambda e: self.say_something())
        self.root.bind("<Control-D>", lambda e: self.say_something())
    ...
    def open_secure_file(self):
        try:
            ...
            elif len(self.sel_file_index_tuple) != 0:
    ...
    def run(self):
        clock = ElapsedTimeClock(self.root, font=('times', 10, 'bold'), bg='white')
        clock.place(relx=1.0, rely=1.0, anchor='se')
        labu = Label(self.root, text=dt.datetime.now().date(), font=('times', 10, 'bold'), bg="WHITE", height=1)
        labu.place(relx=0.0, rely=1.0, anchor='sw')
        self.root.mainloop()
